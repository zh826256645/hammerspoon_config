...
- 工作模式下接通电源时阻止系统因空闲自动休眠，但允许屏幕关闭、主动睡眠和合盖休眠；娱乐模式恢复系统默认策略。
- 设置了 `workModeOnly = true` 的应用快捷键只在工作模式启用。
- 屏幕休眠后的应用、蓝牙和 Wi-Fi 行为按休眠开始时的模式处理。

### 休眠、唤醒与解锁

- 娱乐模式下屏幕持续休眠 15 秒后：关闭设置了 `closeOnSleep = true` 或 `closeOnSleepInEntertainmentMode = true` 的应用，断开配置的蓝牙设备，并关闭蓝牙与 Wi-Fi。
- 工作模式下屏幕休眠不执行应用或设备清理。
